...
public class GameOver : MonoBehaviour  //attached to ?? Object in GameOver Scene
{
    ...
    void Update()
    {
      if(CurrentDisplayScore == FinalGameOverScore)
      {
        if (PointCountUpConcluded == false)
        {
          ...
          else if (FinalGameOverScore >= 50 && FinalGameOverScore < 100)
            {
              SilverSound.Play();
                StartCoroutine(InstantiateSilver());
                PointCountUpConcluded = true;
            }
          else if (FinalGameOverScore >= 100 && FinalGameOverScore < 200)
            {
              GoldSound.Play();
              StartCoroutine(InstantiateGold());
              PointCountUpConcluded = true;
            }
          else if (FinalGameOverScore >= 200)
            {
              ApplauseSound.Play();
              PlatSound.Play();
